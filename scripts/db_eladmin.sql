-- DROP DATABASE IF EXISTS `db_eladmin`;
CREATE DATABASE `db_eladmin`;
use `db_eladmin`;
...
CREATE TABLE `code_gen_config` (
  ...
  `api_alias` varchar(50) DEFAULT NULL COMMENT '接口名称',
...
CREATE TABLE `mnt_app` (
  ...
  `upload_path` varchar(20) DEFAULT NULL COMMENT '上传目录',
  `deploy_path` varchar(50) DEFAULT NULL COMMENT '部署路径',
...
CREATE TABLE `mnt_database` (
  ...
  `jdbc_url` varchar(50) NOT NULL COMMENT 'jdbc连接',
...
CREATE TABLE `sys_quartz_job` (
  `job_id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT 'ID',
  `bean_name` varchar(50) DEFAULT NULL COMMENT 'Spring Bean名称',
  ...
  `sub_task` varchar(50) DEFAULT NULL COMMENT '子任务ID',
...
CREATE TABLE `sys_user` (
  ...
  `avatar_name` varchar(100) DEFAULT NULL COMMENT '头像地址',
  `avatar_path` varchar(100) DEFAULT NULL COMMENT '头像真实路径',
  `password` varchar(100) DEFAULT NULL COMMENT '密码',
...
CREATE TABLE `tool_alipay_config` (
  ...
  `charset` varchar(50) DEFAULT NULL COMMENT '编码',
  `format` varchar(50) DEFAULT NULL COMMENT '类型 固定格式json',
  `gateway_url` varchar(50) DEFAULT NULL COMMENT '网关地址',
  `notify_url` varchar(100) DEFAULT NULL COMMENT '异步回调',
...
CREATE TABLE `tool_email_config` (
  ...
  `host` varchar(50) DEFAULT NULL COMMENT '邮件服务器SMTP地址',
  `pass` varchar(50) DEFAULT NULL COMMENT '密码',
  `port` varchar(8) DEFAULT NULL COMMENT '端口',
  `user` varchar(50) DEFAULT NULL COMMENT '发件者用户名',
  PRIMARY KEY (`config_id`) USING BTREE
...
CREATE TABLE `tool_local_storage` (
  ...
  `real_name` varchar(50) DEFAULT NULL COMMENT '文件真实的名称',
...
CREATE TABLE `tool_qiniu_config` (
  ...
  `bucket` varchar(50) DEFAULT NULL COMMENT 'Bucket 识别符',
  `host` varchar(50) NOT NULL COMMENT '外链域名',
  `secret_key` text DEFAULT NULL COMMENT 'secretKey',
  `type` varchar(50) DEFAULT NULL COMMENT '空间类型',
  `zone` varchar(50) DEFAULT NULL COMMENT '机房',
  PRIMARY KEY (`config_id`) USING BTREE
...
CREATE TABLE `tool_qiniu_content` (
  ...
  `type` varchar(50) DEFAULT NULL COMMENT '文件类型：私有或公开',
