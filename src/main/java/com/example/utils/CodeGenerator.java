...
public class CodeGenerator {
    ...
    static {
        // 必填
        ds.setUrl("jdbc:mysql://localhost:3306/db_test?useUnicode=true&characterEncoding=utf-8&allowMultiQueries=true&useSSL=false&serverTimezone=GMT%2b8");
        ds.setUsername("mysql");
        ds.setPassword("mysql");
    }

    private static final String schemaName = "db_test";   // 数据库名称，必填
    ...
    static List<TableColumn> getTableColumns(String tableName) throws SQLException {
        String sql = "SELECT table_name,column_name,data_type, column_comment, numeric_precision, numeric_scale FROM information_schema.COLUMNS WHERE table_schema = ? and table_name = ?";
        List<Entity> fields = Db.use(ds).query(sql, schemaName, tableName);
